 ...
 * would be skipped-merged), `mainRevertsPr` says main reverted #N, and
 * splitQueue puts gh-18 back in `eligible`.
 */
...
const token = /gh-18/i;
const cover = merged.find((pr) => pr.headRefName === "fix/gh-18" || token.test(pr.body ?? ""));
console.log(
  cover
    ? `merged cover: PR #${cover.number} (${cover.headRefName}) — without the revert guard, gh-18 would be skipped-merged`
    : "NO merged cover found for gh-18 (unexpected)",
);
if (cover) {
...
const ok = split.eligible.some((i) => i.id === "gh-18");
console.log(ok ? "RE-QUEUE PROVEN: gh-18 is eligible again" : "FAILED: gh-18 not re-queued");
